...
public class HelloSoot {
    ...
    public static void main(String[] args) {
        ...
        int c = 1;
        UnitPatchingChain method_body = body.getUnits();
        for (Unit u : method_body) {
        ...
        for (Unit u : body.getUnits()) {
            if (u instanceof JIfStmt){
                ...
                branchList.add(u);
                succ_list = ug.getPredsOf(u);
                for(Unit pred : succ_list){
        ...
        Unit target = method_body.getLast();
        target = method_body.getPredOf(target);
        target = method_body.getPredOf(target);
        System.out.println("\n\n\ntarget unit: " + target);
        ArrayList<Unit> target_path = method_entry_point_to_callsite(sm, target);
        System.out.println("++++++++++++++target path++++++++++++++++\nlength: " + target_path.size());
        for(Unit i : target_path)
    ...
    public static void exec_next_unit(UnitGraph unitgraph, HashMap<Unit, Integer> unit_table, ArrayList<Unit> unit_sequence, Unit current_unit){
        ...
        for(Unit u: succ_list){
            ...
            if(!unit_table.containsKey(next_unit)){
                new_units.add(next_unit);
            }
            else{
                exec_ed_units.add(next_unit);
            }
        }
        ...
        for(Unit unit_to_exec : exec_ed_units){
            ...
            for(int i = 0; i < repeat_instance; i++){
                ...
                for(Unit u : new_units){
                    Unit next_unit = u;
                    unit_table.put(next_unit, unit_sequence.size());
                    unit_sequence.add(next_unit);
                    exec_next_unit(unitgraph, unit_table, unit_sequence, next_unit);
                    unit_table.remove(next_unit);
                    unit_sequence.remove(next_unit);
                }
            }
